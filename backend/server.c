 ...
 */
static void build_message_buffer(struct memory_region *conn) {
    conn->memory_region = malloc(DATA_SIZE);
    memset(conn->memory_region, 0, DATA_SIZE);

    show_memory_map(conn->memory_region);
    ...
    while (rdma_get_cm_event(cm_event_channel, &received_event) == 0) {
        ...
        switch (cm_event.event) {
            ...
            case RDMA_CM_EVENT_ESTABLISHED:
                ...
                poll_for_completion_events(client_res->cq, &wc, 1);
                read_message_buffer(_region);
                while (strcmp(_region->memory_region, "") == 0) {
                    read_message_buffer(_region);
                    sleep(1);
                }
                show_memory_map(_region->memory_region);
                break;
